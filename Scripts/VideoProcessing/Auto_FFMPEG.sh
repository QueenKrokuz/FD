...
for i in *.avi; do 
    ...
    mkdir -p ./SCCichlidVid/Group/ToTrack/$foldername;
    ...
    for n in `seq 6`; do 
        ...
        ffmpeg -i "$i" -ss $start -t $duration -c:v copy -c:a copy ./SCCichlidVid/Group/ToTrack/$foldername/cut"$n"-"$i"
    ...
    mv ./SCCichlidVid/Group/Cropped/$i ./SCCichlidVid/Group/Originals/$i
# End outer for loop
done
